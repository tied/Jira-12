...
def  cleangroupList = groupList.getAt(0).name
...
if(groupList != null){
    ...
    issue.setCustomFieldValue(customFieldManager.getCustomFieldObject('customfield_10302'),cleangroupList )

    issueManager.updateIssue(currentUser, issue, EventDispatchOption.ISSUE_UPDATED, false);

    //ApplicationUser currentUser = ComponentAccessor.getJiraAuthenticationContext().getLoggedInUser()
    //cf.updateValue(null, issue, new ModifiedValue(issue.getCustomFieldValue(cf), cleangroupList), changeHolder);
